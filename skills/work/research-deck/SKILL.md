...
配色與版式規格見 `deck-design-system`，圖表規格見 `research-figures`。
...
**步驟 5 — 佈置佐證。** 照訪談確認的圖→Q 對應表排。要重畫的圖從資料產生，要貼的圖用 `E20`／`E13`／`E14`。版式選擇見 `reference/layouts.md`。

**步驟 6 — 自檢，然後才產檔。** 見第 7 節。

**步驟 7 — 產出。** 見 `reference/emit.md`。
...
- `deck-design-system` — 色票、字階、格線、字體，以及每個版式的精確幾何
